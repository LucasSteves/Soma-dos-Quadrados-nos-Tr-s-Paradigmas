...
```
Main.java             OO - classe SomaQuadrados + classe Main
soma_quadrados.pl     Lógico - Prolog
SomaFuncional.java    Funcional - Java com Streams
```
...
**Comunica a intenção mais rápido de ler:** o funcional. O Prolog vem logo atrás — as duas cláusulas
já são a definição do problema. O OO é o que demora mais, precisa abrir o método `calcular()` pra
...
| Lógico (Prolog)       | 4                  | fato/cláusula, unificação de lista, recursão, operador `is`              |
| Funcional (Streams)   | 6                  | `stream()`, lambda, `map`, `reduce`, `Integer::sum`                      |
